...
template <class node_t_, class weight_t_ = bool> class AdjacencySet {
public:
...
template <class node_t_, class weight_t_ = bool> class DirectedAdjacencySet {
public:
  using node_t = node_t_;
  using weight_t = weight_t_;
  using extra_data_t = void *;
...
public:
  ...
  DirectedAdjacencySet(node_t n) : out_nodes(n), in_nodes(n) {}
...
int main(int32_t argc, char *argv[]) {
  ...
  if (symetric) {
    ...
    if (!use_weights) {
      auto edges = get_edges_from_file_adj(graph_filename, &edge_count,
                                           &node_count, symetric);
      auto g = DirectedAdjacencySet<uint64_t>(node_count);
      uint64_t start = get_usecs();
      ...
      auto edges = get_edges_from_file_adj<uint32_t, weight_type>(
          graph_filename, &edge_count, &node_count, symetric);
      auto g = DirectedAdjacencySet<uint64_t, weight_type>(node_count);
      uint64_t start = get_usecs();
